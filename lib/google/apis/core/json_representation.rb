      ...
      module JsonRepresentationSupport
        ...
        module JsonSupport
          ...
          def set_default_options(name, options)
            if options[:base64]
              options[:render_filter] = ->(value, _options) { value.nil? ? nil : Base64.urlsafe_encode64(value) }
              options[:parse_filter] = ->(fragment, _options) { Base64.urlsafe_decode64(fragment) }
            end
            if options[:type] == DateTime
              options[:render_filter] = ->(value, _options) { value.nil? ? nil : value.is_a?(DateTime) ? value.rfc3339(3) : value.to_s }
              options[:parse_filter] = ->(fragment, _options) { DateTime.parse(fragment) }
            end
